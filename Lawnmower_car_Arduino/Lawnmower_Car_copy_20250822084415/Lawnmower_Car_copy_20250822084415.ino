...
//const uint8_t BLADE_PWM_REV = 11;   // PWM-capable
...
static uint16_t _acc1 = 0, _acc2 = 0, _acc4 = 0;
...
void enableAllDrivers() {
  ...
  if (BLADE_STBY != 255) {
    pinMode(BLADE_STBY, OUTPUT);
    digitalWrite(BLADE_STBY, HIGH);
  }
}
...
void calibrateRCNeutral() {
  ...
  Serial.print(F("Auto-center trims: steer="));
  Serial.print(STEER_TRIM, 3);
  Serial.print(F(" th_add="));
  Serial.print(TH_ADD_TRIM, 3);
  Serial.print(F(" spin="));
  Serial.println(SPIN_TRIM, 3);
}
...
void setup() {
  ...
  Serial.print(F("Modes: USE_PID="));
  Serial.print((int)USE_PID);
  Serial.print(F(", USE_FEEDFORWARD="));
  Serial.println((int)USE_FEEDFORWARD);
  Serial.println();
}
...
void loop() {
  if (killActive()) {
    ...
    if (millis() - t0 > 500) {
      t0 = millis();
      Serial.println(F("[KILL] D52 LOW — holding."));
    }
    delay(10);
    return;
  }
  ...
  static float ch1f = 0;
  ch1f = (1 - ALPHA_STEER) * ch1f + ALPHA_STEER * raw_steer;
  ch1f -= STEER_TRIM;
  if (fabs(ch1f) < DB_STEER) ch1f = 0.0f;
  ch1_steer = clampf(ch1f, -1.0f, 1.0f);

  float raw_th_add = clampf(mapf(chRaw[1], 1000, 2000, -0.5f, +0.5f), -0.5f, 0.5f) * RC_POL_CH2_TH_ADD;
  static float ch2f = 0;
  ch2f = (1 - ALPHA_TH_ADD) * ch2f + ALPHA_TH_ADD * raw_th_add;
  ch2f -= TH_ADD_TRIM;
  if (fabs(ch2f) < DB_TH_ADD) ch2f = 0.0f;
  ch2_th_add = clampf(ch2f, -0.5f, 0.5f);
  ...
  static float ch3f = 0;
  ch3f = (1 - ALPHA_TH_BASE) * ch3f + ALPHA_TH_BASE * raw_th_base;
  if (ch3f < DB_TH_BASE) ch3f = 0.0f;
  ch3_th_base = clampf(ch3f, 0.0f, 0.5f);

  float raw_spin = clampf(mapf(chRaw[3], 1000, 2000, -1.0f, 1.0f), -1.0f, 1.0f) * RC_POL_CH4_SPIN;
  static float ch4f = 0;
  ch4f = (1 - ALPHA_SPIN) * ch4f + ALPHA_SPIN * raw_spin;
  ch4f -= SPIN_TRIM;
  if (fabs(ch4f) < DB_SPIN) ch4f = 0.0f;
  ch4_spin = clampf(ch4f, -1.0f, 1.0f);
  ...
  float SP[4] = { sp_left, sp_right, sp_left, sp_right };
  int U[4] = { 0, 0, 0, 0 };

  // Compute raw control per wheel (PID or open-loop)
  for (uint8_t w = 0; w < 4; w++) {
#if USE_PID
    U[w] = wheelPID(w, SP[w], RPM[w], dt);  // signed PWM
#else
    float m = clampf(SP[w] / MAX_WHEEL_RPM, -1.0f, 1.0f);
        ...
        if (stall_accum_ms[w] >= STALL_MS) {
          ...
          stall_accum_ms[w] = 0;
            ...
            if (millis() - lastKickLogMs[w] > 250) {  // rate-limit
              ...
              Serial.print(F("[KICK] w="));
              Serial.print(w);
              Serial.print(F(" SP="));
              Serial.print(SP[w], 1);
              Serial.print(F(" RPM="));
              Serial.print(RPM[w], 1);
              Serial.print(F(" U="));
              Serial.print(U[w]);
              Serial.print(F(" -> "));
              Serial.print(sgnf(SP[w]) * KICK_PWM);
              Serial.println();
            }
          #endif
        }
  ...
  for (uint8_t w = 0; w < 4; w++) {
    if (fabs(SP[w]) >= SP_ZERO_RPM) {
      // minimum bias to overcome stiction
      if (abs(U[w]) < DZ_BIAS_PWM) U[w] = sgnf(SP[w]) * DZ_BIAS_PWM;

      // kick if scheduled
      if ((long)(kick_until_ms[w] - millis()) > 0) {
        int want = sgnf(SP[w]) * KICK_PWM;
        if (abs(U[w]) < abs(want)) U[w] = want;
#if USE_PID
        // optional: bleed integrator a bit during kick to avoid overshoot
        Iterm[w] *= 0.95f;
#endif
      }
    } else {
      // near zero: enforce idle cutoff
      if (abs(U[w]) < PWM_IDLE_CUTOFF) U[w] = 0;
    }
  }

  // Slip limiting per side: (0↔2) left; (1↔3) right
  auto limitSlipPair = [&](uint8_t a, uint8_t b) {
    // Same-direction movement only
    if (sgnf(SP[a]) == 0 || sgnf(SP[a]) != sgnf(SP[b])) return;
    ...
    if (diff > SLIP_RPM) {
      #if TRACTION_DEBUG
// sideIdx: 0 = left pair (0,2), 1 = right pair (1,3)
uint8_t sideIdx = (a == 0 && b == 2) ? 0 : 1;
if (millis() - lastSlipLogMs[sideIdx] > 200) {   // rate-limit
  ...
  Serial.print(F(" boost→"));       Serial.println(U[slow]);
      ...
      // Cap the fast wheel
      int capped = sgnf(U[fast]) * (int)SLIP_CAP_PWM;
      if (abs(U[fast]) > abs(capped)) {
        U[fast] = capped;
#if USE_PID
        Iterm[fast] *= 0.95f;  // bleed integrator when we’re clipping
#endif
      }
      // Nudge the slow wheel
      U[slow] = constrain(U[slow] + sgnf(SP[slow]) * (int)SLOW_BOOST_PWM, -255, 255);
    }
  };
  ...
  if (millis() - lastPrint >= 250) {
    ...
    for (int i = 0; i < 6; i++) {
      Serial.print(chRaw[i]);
      if (i < 5) Serial.print(' ');
    }
    Serial.print(F(" | RC: s="));
    Serial.print(ch1_steer, 2);
    Serial.print(F(" tb="));
    Serial.print(ch3_th_base, 2);
    Serial.print(F(" ta="));
    Serial.print(ch2_th_add, 2);
    Serial.print(F(" th="));
    Serial.print(th_comb, 2);
    Serial.print(F(" sp="));
    Serial.print(ch4_spin, 2);
    Serial.print(F(" | SP[L/R]=("));
    Serial.print(sp_left, 1);
    Serial.print('/');
    Serial.print(sp_right, 1);
    Serial.print(')');
    Serial.print(F("  RPM[L/R]=("));
    Serial.print(RPM[0], 1);
    Serial.print('/');
    Serial.print(RPM[1], 1);
    Serial.print(')');
    Serial.print(F("  Blade="));
    Serial.print(ch5_blade ? ch6_blade_pwm : 0);
