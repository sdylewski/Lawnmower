...
DEBOUNCE_US = 15              # post-edge holdoff (us)
...
@asm_pio()
def edge_counter_chunk_db_bistable():
    # Pull #1: chunk target N -> store in ISR (constant) and Y (countdown)
    pull()
    mov(isr, osr)          # ISR = N (constant)
    mov(y,   osr)          # Y   = N (chunk countdown)

    # Pull #2: debounce cycles (µs at SM_FREQ_HZ) -> keep in OSR; X is working copy
    pull()
    mov(x,   osr)          # X = deb_cycles (working)
    # OSR now holds deb_cycles as a constant for reloads

    wrap_target()

    # --- Require LOW to be stable for deb_cycles before arming ---
    label("qual_low_reload")
    mov(x, osr)            # reload X = deb_cycles
    label("qual_low")
    jmp(pin, "qual_low_reload")  # if it went HIGH too soon, restart the low-qual
    jmp(x_dec, "qual_low")       # burn deb_cycles while LOW stays LOW

    # --- Rising edge ---
    wait(1, pin, 0)        # true rising edge

    # --- Require HIGH to remain stable for deb_cycles (post-edge qualification) ---
    mov(x, osr)            # reload X = deb_cycles
    label("qual_high")
    jmp(pin, "dec_high")   # stay here while HIGH
    jmp("qual_low_reload") # if it dropped early -> discard this edge and start over
    label("dec_high")
    jmp(x_dec, "qual_high")# burn deb_cycles while HIGH

    # --- Edge accepted only if both LOW-before and HIGH-after were stable ---
    jmp(y_dec, "done_edge")# Y-- ; if not zero, just loop (no IRQ)
    irq(rel(0))            # N qualified edges -> raise IRQ0
    mov(y, isr)            # reload Y = N

    label("done_edge")
    jmp("qual_low_reload") # re-arm for next qualified edge
...
def init_sms(CHUNK_N):
    ...
    for i, (wheel, pin_num) in enumerate(encoder_pins.items()):
        ...
        sm  = StateMachine(
            i,
            edge_counter_chunk_db_bistable,   # <— use this stricter program
            freq=SM_FREQ_HZ,
            in_base=pin
        )
