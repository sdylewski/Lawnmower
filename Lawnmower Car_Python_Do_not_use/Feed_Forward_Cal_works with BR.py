...
# Debounce + chunking
CHUNK_LIST  = [1, 32]   # try 1/2/4; edges should stay ~constant; IRQs drop ~1/n
...
WHEELS = ["FL"]           # connect only BR for your bench run; add others later
...
# Count IRQs only; derive edges = IRQs*CHUNK + remainder
...
def init_sms(CHUNK_N):
    ...
    for i, (wheel, pin_num) in enumerate(encoder_pins.items()):
        ...
        sm  = StateMachine(
            ...
            jmp_pin=pin        # REQUIRED for jmp(pin, ...) to read THIS GPIO
        )
...
def sweep_fwd(wheel, CHUNK_N):
    ...
    for pwm_val in range(0, MAX_PWM+1, PWM_STEP):
        ...
        # Align chunk phase to start-of-window (Y := N)
        sm = sms[wheel]
        sm.active(0)
        ...
        # Optional debug:
        # if di > 0: print("   diag: IRQs=%d  rem=%d  edges/IRQ=%.2f" % (di, remainder, dp_edges/di))
